...
/**
 * Omnilingual ASR 300M int8 (1600+ languages including Persian) — stable on phones (full FP model OOMs on many devices).
 * ~350 MB. Multilingual CTC; includes Persian.
 */
object OmnilingualEngine {

    private const val TAG = "OmnilingualEngine"
    private const val FOLDER = "omnilingual-300m-int8"
    ...
    private fun modelDir(context: Context): File =
        File(context.applicationContext.filesDir, "omni-models/$FOLDER")

    private fun findModelFile(dir: File): File? {
        val names = listOf("model.int8.onnx", "model.onnx")
        for (name in names) {
            val f = File(dir, name)
            if (f.exists() && f.length() > 50_000_000) return f
        }
        dir.walkTopDown().maxDepth(3).forEach { f ->
            if (f.isFile && f.name.endsWith(".onnx") && f.length() > 50_000_000) return f
        }
        return null
    }
    ...
    fun ensureModel(context: Context, onProgress: (Int) -> Unit = {}) {
        ...
        try {
            val root = File(context.applicationContext.filesDir, "omni-models")
