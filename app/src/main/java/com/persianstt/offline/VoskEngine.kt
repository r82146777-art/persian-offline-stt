...
object VoskEngine {
        ...
        BufferedInputStream(conn.inputStream).use { input ->
            FileOutputStream(zipFile).use { out ->
                ...
                while (true) {
                    ...
                    val pct = ((done * 80) / total).toInt().coerceIn(0, 80)
    ...
    fun transcribe(pcm16: ShortArray, sampleRate: Int = 16000): String {
        val model = if (loadedLang == "en") enModel else faModel
        if (model == null || pcm16.isEmpty()) return ""
        if (pcm16.size < sampleRate / 4) return ""
        return try {
